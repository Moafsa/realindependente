...
@section('title', 'Editar Profissional - ' . $coach->name)
...
<div class="animate__animated animate__fadeIn">
    ...
    <div class="mb-12 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-black text-white tracking-tight">Editar Profissional</h1>
            <p class="text-gray-400 mt-2">Atualize as credenciais e acordos de <b>{{ $coach->name }}</b>.</p>
        </div>
    ...
    <form action="{{ route('admin.coaches.update', $coach) }}" method="POST" enctype="multipart/form-data" class="max-w-4xl">
        @csrf
        @method('PUT')
        <div class="space-y-8">
            <!-- Basic Info Card -->
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $coach->name) }}" required
                               class="w-full px-6 py-4 bg-white/5 border border-white/10 rounded-2xl text-sm text-white focus:ring-2 focus:ring-indigo-500 transition-all outline-none">
                        @error('name') <p class="text-rose-500 text-[10px] font-bold mt-1 ml-2 uppercase tracking-tighter">{{ $message }}</p> @enderror
                    </div>

                    <div class="space-y-2">
                        <label for="email" class="block text-[10px] font-black text-gray-500 uppercase tracking-widest ml-2">E-mail Institucional</label>
                        <input type="email" name="email" id="email" value="{{ old('email', $coach->email) }}" required
                               class="w-full px-6 py-4 bg-white/5 border border-white/10 rounded-2xl text-sm text-white focus:ring-2 focus:ring-indigo-500 transition-all outline-none">
                        @error('email') <p class="text-rose-500 text-[10px] font-bold mt-1 ml-2 uppercase tracking-tighter">{{ $message }}</p> @enderror
                    </div>

                    <div class="space-y-2">
                        <label for="password" class="block text-[10px] font-black text-gray-500 uppercase tracking-widest ml-2">Nova Senha (opcional)</label>
                    ...
                    <div class="space-y-2">
                        <label for="password_confirmation" class="block text-[10px] font-black text-gray-500 uppercase tracking-widest ml-2">Confirmar Nova Senha</label>
                    ...
                    <div class="space-y-2">
                        <label for="avatar" class="block text-[10px] font-black text-gray-500 uppercase tracking-widest ml-2">Alterar Foto</label>
                        <div class="flex items-center gap-4">
                            <div class="relative">
                                <img src="{{ $coach->avatar_url }}" class="w-14 h-14 rounded-xl object-cover ring-2 ring-white/5">
                                <div class="absolute -bottom-1 -right-1 w-4 h-4 bg-indigo-600 border-2 border-[#0F1423] rounded-full"></div>
                            </div>
            ...
            <div class="bg-[#0F1423] rounded-[2.5rem] p-10 shadow-2xl border border-white/5 relative overflow-hidden group">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        ...
                        <input type="number" step="0.01" name="salary" id="salary" value="{{ old('salary', $coach->salary) }}"
                               class="w-full px-6 py-4 bg-white/5 border border-white/10 rounded-2xl text-sm text-white focus:ring-2 focus:ring-indigo-500 transition-all outline-none">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="payment_frequency" id="payment_frequency"
                            ...
                            @foreach($frequencies as $key => $label)
                                <option value="{{ $key }}" {{ old('payment_frequency', $coach->payment_frequency) == $key ? 'selected' : '' }} class="bg-[#0F1423]">{{ $label }}</option>
                            @endforeach
                        </select>
